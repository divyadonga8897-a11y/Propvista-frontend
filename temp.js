...
__export(mockData_exports, {
  MOCK_APARTMENTS: () => MOCK_APARTMENTS,
  MOCK_FLATS: () => MOCK_FLATS,
  STATUS_COLORS: () => STATUS_COLORS,
  getApartmentById: () => getApartmentById,
  getFlatById: () => getFlatById,
  getFlatsForApartment: () => getFlatsForApartment
});
...
var STATUSES = ["Available", "Available", "Available", "Held", "Available", "Rented", "Available", "Sold"];
function generateFlats(floorId, floorNum, aptId, aptName, aptIdx) {
  ...
  return Array.from({ length: 8 }, (_, i) => {
    ...
    const priceBuy = 35e5 + floorNum * 1e5 + i * 5e4 + aptIdx * 2e5;
    const priceRent = 8e3 + floorNum * 500 + i * 200 + aptIdx * 1e3;
...
function generateFloors(aptId, aptName, aptIdx, totalFloors) {
...
var MOCK_APARTMENTS = [
  ...
  },
  {
    ...
    nearby_places: [
      { id: "nb-gv-1", place_name: "Sri Krishna Degree College", category: "School", distance: 1 },
  ...
  },
  {
    ...
    description: "A premium luxury development offering panoramic views of Nandyal's skyline. Skyline Residency redefines urban living with world-class finishes and smart home integrations.",
...
var MOCK_FLATS = MOCK_APARTMENTS.reduce((acc, apt) => {
  apt.floors.forEach((floor) => {
    floor.flats.forEach((flat) => acc.push(flat));
  });
  return acc;
}, []);
var getApartmentById = (id) => MOCK_APARTMENTS.find((a) => a.id === id);
var getFlatById = (id) => MOCK_FLATS.find((f) => f.id === id);
var getFlatsForApartment = (aptId) => MOCK_FLATS.filter((f) => f.apartment_id === aptId);
var STATUS_COLORS = {
  Available: "#22c55e",
  Held: "#f59e0b",
  Booked: "#3b82f6",
