...
import {
  ArrowLeft, Heart, Layers, Compass, Ruler, Home, Bed, Bath, Key,
} from "lucide-react";
import Footer from "@/components/Footer";

export default function NestedFlatDetailPage({
  params,
}: {
  params: Promise<{ id: string; flatId: string }>;
}) {
  ...
  if (!flat) {
    return (
      <div className="flex flex-col min-h-screen items-center justify-center p-8 bg-slate-50">
        ...
        <Link
          href={`/properties/${resolvedParams.id}`}
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 w-full">
        ...
        >
          <ArrowLeft className="h-4 w-4" /> Back to {flat.apartment_name || "Apartment"}
        </Link>
      </div>
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <div className="flex items-center justify-between gap-4 mb-4">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">
                    {flat.apartment_name} · Floor {flat.floor_number}
                  </p>
                </div>
              ...
              <p className="text-xs text-slate-600 leading-relaxed border-t border-slate-50 pt-4">
                {flat.long_description ||
                  flat.short_description ||
                  "Spacious, highly-ventilated, luxury apartment featuring modular fittings and modern high-end specifications."}
              </p>
            </div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <h2 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">
                Pricing & Financial Outlay
              </h2>
              <div className="space-y-3">
                <div className="flex justify-between items-center text-xs">
                  <span className="text-slate-500">Outright Buy Price</span>
                  <span className="font-extrabold text-slate-900">
                    {flat.price_buy ? formatPrice(flat.price_buy) : "N/A"}
                  </span>
                </div>
                <div className="flex justify-between items-center text-xs">
                  <span className="text-slate-500">Monthly Rent</span>
                  <span className="font-extrabold text-slate-900">
                    {flat.price_rent ? `${formatPrice(flat.price_rent)}/mo` : "N/A"}
                  </span>
                </div>
